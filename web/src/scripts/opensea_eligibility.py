...
def check_gtd_wl(address: str, private_key: str, slug: str) -> dict:
    ...
    try:
        ...
        for i, stage in enumerate(stages):
            ...
            # Coba ambil nama stage dari response GraphQL
            stage_name = stage.get("name") or stage.get("label") or stage.get("title") or stage.get("id")
            
            if not stage_name or stage_name == "Erc721SeaDropV1Stage":
                # Jika tidak ada nama dari GraphQL, gunakan fallback
                if len(stages) == 4:
                    custom_names = ["TEAM", "GTD", "FCFS", "PUBLIC"]
                    stage_name = custom_names[i] if i < len(custom_names) else f"Stage {i+1}"
                elif len(stages) == 3:
                    custom_names = ["GTD", "FCFS", "PUBLIC"]
                    stage_name = custom_names[i] if i < len(custom_names) else f"Stage {i+1}"
                else:
                    stage_name = f"Stage {i+1}"

            status = "✅ ELIGIBLE" if is_eligible else "❌ NOT eligible"
            print(f"\n  Stage {i+1} ({stage_name}) → {status}")
            if has_merkle:
...
Examples:
  ...
  python opensea_eligibility.py --private-key 0x... --slug collection-slug --method server
  
  # Multiple Wallets check from file
  python opensea_eligibility.py --file pk.txt --slug collection-slug --method server

  # Full check (all methods)
  python opensea_eligibility.py --private-key 0x... --nft-address 0x... --slug collection-slug --api-key YOUR_KEY
        """,
    )
    parser.add_argument("--address", help="Wallet address (0x...) - optional if private key is provided")
    parser.add_argument("--private-key", help="Wallet private key (0x...) — needed for GTD/WL SIWE")
    parser.add_argument("--file", default="pk.txt", help="File containing list of private keys (one per line). Default: pk.txt")
    ...
    parser.add_argument("--method", choices=["auto", "onchain", "server"], default="server",
                        help="auto=try all, onchain=SeaDrop only, server=SIWE+GraphQL only. Default: server")
    args = parser.parse_args()

    if not args.slug:
        try:
            args.slug = input("Masukkan slug koleksi (nama koleksi): ").strip()
        except KeyboardInterrupt:
            print("\nDibatalkan.")
            sys.exit(1)
        if not args.slug:
            print("❌ Nama koleksi tidak boleh kosong.")
            sys.exit(1)

    if not args.address and not args.private_key and not args.file:
        parser.error("You must provide --address, --private-key, or --file")
        ...
        try:
            with open(args.file, 'r') as f:
                for line in f:
                    ...
                    if not line:
                        continue
                    # Remove " | GUARANTEED" or anything after | or space
                    pk = line.split('|')[0].strip().split()[0].strip()
                    if pk:
                        pks_to_check.append(pk)
        except Exception as e:
            print(f"❌ Error reading file {args.file}: {e}")
            return
    elif args.private_key:
        pks_to_check.append(args.private_key)
    
    if not pks_to_check and args.address:
        # Check single address without PK (onchain/api v2 only)
        pks_to_check = [None] 

    for idx, pk in enumerate(pks_to_check):
        address = args.address
        if pk:
            try:
                # Add 0x prefix if missing for eth_account
                if not pk.startswith("0x"):
                    pk_with_prefix = "0x" + pk
                else:
                    pk_with_prefix = pk
                address = Account.from_key(pk_with_prefix).address
            except Exception as e:
                print(f"\n❌ Invalid private key format at index {idx+1}")
                continue

        print(f"\n{'='*60}")
        print(f"  [{idx+1}/{len(pks_to_check)}] OpenSea Drop Eligibility Checker")
        print(f"  Wallet: {address}")
        if args.slug:
        ...
        if args.method in ("auto", "onchain") and args.nft_address:
            ...
            check_onchain(address, args.nft_address)
            print()
        ...
        if args.method in ("auto", "server") and pk and args.slug:
            print("─ Method 3: SIWE + GraphQL (GTD/WL) ─")
            check_gtd_wl(address, pk, args.slug)
            print()

        if args.method == "onchain" and not args.nft_address:
            print("❌ On-chain method needs --nft-address")
        if args.method == "server" and not pk:
            print("❌ Server method needs --private-key or --file")
        if args.method == "server" and not args.slug:
            print("❌ Server method needs --slug")

        # Sleep between checks to avoid rate limits when processing multiple wallets
        if len(pks_to_check) > 1 and idx < len(pks_to_check) - 1:
